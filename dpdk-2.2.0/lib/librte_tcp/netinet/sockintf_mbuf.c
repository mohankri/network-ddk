...
#include <port/netbsd_stdio.h>
#include <port/netbsd_stdlib.h>
...
ssize_t sowritev(struct socket *so, struct iovec *iovp, int iovcnt)
{
	...
	#endif
	for (i = 0; i < iovcnt; i++, iovp++) {
		struct mbuf *m = m_get(0, MT_HEADER);

		MEXTADD(m, iovp->iov_base, iovp->iov_len, 0, dummy_free, NULL);
		m->m_len = iovp->iov_len;
		m->m_pkthdr.len = iovp->iov_len;
		if (so->so_send) {
			rc = so->so_send(so, NULL, NULL, m, NULL, 0);
			if (rc != 0) {
				return -rc;
			}
		}
